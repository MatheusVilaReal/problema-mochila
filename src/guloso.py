...
currentNode = totalWeight = totalInterest = solutionInterest = counter = elapsedTime = 0
...
for i in range(n):
    print("x[" + str(i+1) + "] =\tInteresse: ", items[i].interest, "\t| Peso: ", items[i].weight, "\t| Custo-benefício: ", items[i].interest/items[i].weight)
...
while(elapsedTime < 15):
    ...
    while(Arvore[currentNode].data == 0 and currentNode > 0):

        counter += 1
        nodeBuffer = currentNode
        currentNode = Arvore[currentNode].predecessor(0)
        Arvore.remove_node(nodeBuffer)
...
if(filename):
    backpack.genCsvNoSort(filename, txt, n, elapsedTime, solutionInterest, solutionWeight, sys.argv, capacity, solution)
